...
public class Main {
    public static void main(String[] args) {
        ...
        for(int i = 0; i<clients.length; i++){
            ...
            if(i == 2 || i == 3){
                chambres[i] = new Chambre(i, i+1, i, true, "Double", hotel);

            }
            chambres[i] = new Chambre(i, i+1, i, false, "Simple", hotel);
            reservations[i] = new Reservation("2026-04-21", "2026-05-21", hotel, chambres[i], clients[i]);
            products[i] = new Product(("Produit " + i), (5.2 + i), (2 + i));
        }
        sejours[0] = new Sejour(reservations[1], hotel);
        sejours[1] = new Sejour(reservations[2], hotel);
        sejours[1].getConsommationMinibar().addProduit(products[0]);
        sejours[1].getConsommationMinibar().addProduit(products[1]);
        sejours[2] = new Sejour(reservations[3], hotel);
        sejours[2].getConsommationMinibar().addProduit(products[2]);
        sejours[2].getConsommationMinibar().addProduit(products[3]);
